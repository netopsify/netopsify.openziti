...
description:
  ...
  - Controls which identities can access which edge routers.
  - Essential for defining network ingress/egress points for identities.
options:
  ziti_controller_url:
    description:
  ...
  edge_router_roles:
    description:
      ...
      - e.g., ['@router-01', '#public-routers', '#all'].
    required: true
    type: list
    elements: str
  identity_roles:
    description:
      - A list of identity roles to include in this policy.
      - e.g., ['@my-device', '#employees', '#all'].
    required: true
    type: list
    elements: str
  semantic:
...
EXAMPLES = r'''
- name: Allow all identities to access all routers (public policy)
  openziti_router_policy:
    ziti_controller_url: "https://ziti.example.com"
    ziti_username: "admin"
    ziti_password: "password"
    policy_name: "all-routers-public"
    edge_router_roles: ["#all"]
    identity_roles: ["#all"]
    state: present

- name: Restrict specific identities to a specific router
  openziti_router_policy:
    ziti_controller_url: "https://ziti.example.com"
    ziti_username: "admin"
    ziti_password: "password"
    policy_name: "west-coast-access"
    edge_router_roles: ["@router-west"]
    identity_roles: ["#sales-west"]
    semantic: "AnyOf"
    state: present

- name: Remove a router policy
  openziti_router_policy:
    ziti_controller_url: "https://ziti.example.com"
    ziti_username: "admin"
    ziti_password: "password"
...
RETURN = r'''
policy:
  description: The full dictionary of the router policy object.
  returned: success
  type: dict
  contains:
    id:
...
def main():
    """
    Main entry point for the OpenZiti Edge Router Policy module.
    """
    ...
    if state == 'present':
        if existing_policy:
            ...
            if needs_update:
                ...
                update_data = OpenZitiRouterPolicyCreate(
                    name=policy_name,
                    edgeRouterRoles=edge_router_roles,
                    identityRoles=identity_roles,
                    semantic=semantic
                )
                client.update_router_policy(existing_policy.id, update_data)
                result['changed'] = True
                updated_policy = client.get_router_policy_by_name(policy_name)
